...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-ink p-6 relative">
      ...
      <div className="absolute top-1/4 right-1/4 w-96 h-96 bg-coral/10 rounded-full blur-3xl pointer-events-none"></div>
      <div className="absolute bottom-1/4 left-1/4 w-96 h-96 bg-teal/10 rounded-full blur-3xl pointer-events-none"></div>
      ...
      <div className="w-full max-w-md bg-paper/5 border border-lavender/15 rounded-3xl p-8 backdrop-blur-md relative overflow-hidden shadow-2xl">
        <div className="flex flex-col items-center mb-6 text-center">
          ...
          <h2 className="text-3xl font-extrabold tracking-tight text-white font-fraunces">Join Rogue</h2>
          <p className="text-xs text-lavender/80 mt-1">Single Sign-On with your Google Account</p>
        </div>

        {/* Feature & Access Policy Explanation */}
        <div className="mb-6 bg-[#1D202D] p-4 rounded-2xl border border-[#2B2F42] space-y-3 text-xs text-lavender">
          <div className="flex items-center gap-2 text-white font-bold">
            <GraduationCap className="w-4 h-4 text-coral" /> Institute Email Policy
          </div>
          <ul className="space-y-2 text-[11px] leading-relaxed text-lavender/80">
            <li className="flex items-start gap-2">
              <span className="text-teal font-bold">•</span>
              <span><strong>College Email Required (<code className="text-teal">mits.ac.in</code>):</strong> Registration is strictly gated to verified college email addresses to ensure campus safety.</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-rose-400 font-bold">•</span>
              <span><strong>Personal Accounts (Gmail / Yahoo):</strong> Not permitted for registration. Please sign in with your official college Google account.</span>
            </li>
          </ul>
        </div>
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <span>{loading ? 'Connecting Google Account...' : 'Sign up with Google Account'}</span>
          </button>

          <div className="flex items-center gap-2 text-[11px] text-lavender/60 justify-center">
            <CheckCircle2 className="w-3.5 h-3.5 text-teal" /> Secure Single Sign-On (OAuth 2.0)
          </div>
        </div>

        <p className="text-center text-xs text-lavender/60 mt-8">
          Already registered?{' '}
